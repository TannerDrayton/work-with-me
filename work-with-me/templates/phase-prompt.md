...
> **Note:** Planning methodology is in `agents/wwm-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/work-with-me/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/wwm:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/work-with-me/references/tdd.md` for TDD plan structure.
...
See `~/.claude/work-with-me/templates/user-setup.md` for full schema and examples
...
See `~/.claude/work-with-me/workflows/verify-phase.md` for verification logic.
