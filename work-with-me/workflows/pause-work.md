...
```bash
timestamp=$(node ~/.claude/work-with-me/bin/wwm-tools.js current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
