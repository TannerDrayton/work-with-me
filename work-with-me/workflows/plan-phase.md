<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates wwm-phase-researcher, wwm-planner, and wwm-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node ~/.claude/work-with-me/bin/wwm-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
**If `planning_exists` is false:** Error — run `/wwm:new-project` first.
...
```bash
PHASE_INFO=$(node ~/.claude/work-with-me/bin/wwm-tools.js roadmap get-phase "${PHASE}")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WWM ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn wwm-phase-researcher

```bash
PHASE_DESC=$(node ~/.claude/work-with-me/bin/wwm-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/work-with-me/bin/wwm-tools.js state-snapshot)
...
<phase_context>
IMPORTANT: If CONTEXT.md exists below, it contains user decisions from /wwm:discuss-phase.
...
```
Task(
  prompt="First, read ~/.claude/agents/wwm-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
## 8. Spawn wwm-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 WWM ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Phase Context:**
IMPORTANT: If context exists below, it contains USER DECISIONS from /wwm:discuss-phase.
...
<downstream_consumer>
Output consumed by /wwm:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ~/.claude/agents/wwm-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn wwm-plan-checker Agent
...
Task(
  ...
  subagent_type="wwm-plan-checker",
  model="{checker_model}",
...
- /wwm:plan-phase {X} --research — re-research first
...
- [ ] wwm-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] wwm-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] wwm-plan-checker spawned with CONTEXT.md
