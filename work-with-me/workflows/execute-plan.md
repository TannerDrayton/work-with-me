...
@~/.claude/work-with-me/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/work-with-me/bin/wwm-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="wwm-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn wwm-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
See `~/.claude/work-with-me/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/work-with-me/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/work-with-me/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/work-with-me/templates/summary.md`.
...
Update STATE.md using wwm-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/work-with-me/bin/wwm-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/work-with-me/bin/wwm-tools.js state update-progress

# Record execution metrics
node ~/.claude/work-with-me/bin/wwm-tools.js state record-metric \
...
node ~/.claude/work-with-me/bin/wwm-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/work-with-me/bin/wwm-tools.js state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using wwm-tools:

```bash
node ~/.claude/work-with-me/bin/wwm-tools.js state record-session \
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/wwm:execute-phase {phase}` + `/wwm:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/wwm:plan-phase {Z+1}` + `/wwm:verify-work {Z}` + `/wwm:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/wwm:complete-milestone` + `/wwm:verify-work` + `/wwm:add-phase` |
