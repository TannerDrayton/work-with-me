...
Detect whether WWM is installed locally or globally by checking both locations:

```bash
# Check local first (takes priority)
if [ -f "./.claude/work-with-me/VERSION" ]; then
  cat "./.claude/work-with-me/VERSION"
  echo "LOCAL"
elif [ -f ~/.claude/work-with-me/VERSION ]; then
  cat ~/.claude/work-with-me/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
⚠️  **Note:** The installer performs a clean install of WWM folders:
- `commands/gsd/` will be wiped and replaced
- `work-with-me/` will be wiped and replaced
- `agents/wwm-*` files will be replaced

(Paths are relative to your install location: `~/.claude/` for global, `./.claude/` for local)

Your custom files in other locations are preserved:
- Custom commands not in `commands/gsd/` ✓
- Custom agents not prefixed with `wwm-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any WWM files directly, they'll be automatically backed up to `wwm-local-patches/` and can be reapplied with `/wwm:reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
rm -f ./.claude/cache/wwm-update-check.json
```
...
Check for wwm-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /wwm:reapply-patches to merge your modifications into the new version.
```
