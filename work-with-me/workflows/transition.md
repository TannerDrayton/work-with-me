...
**Delegate ROADMAP.md and STATE.md updates to wwm-tools:**

```bash
TRANSITION=$(node ~/.claude/work-with-me/bin/wwm-tools.js phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `wwm-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/work-with-me/bin/wwm-tools.js progress bar --raw)
```
...
**Use the transition result from `wwm-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/wwm:plan-phase [X+1]")
...
**Also available:**
- `/wwm:discuss-phase [X+1]` — gather context first
- `/wwm:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/wwm:complete-milestone {version}")
