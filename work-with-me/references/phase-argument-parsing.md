...
```bash
PHASE_INFO=$(node ~/.claude/work-with-me/bin/wwm-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/work-with-me/bin/wwm-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/work-with-me/bin/wwm-tools.js find-phase "${PHASE}" --raw)
```
