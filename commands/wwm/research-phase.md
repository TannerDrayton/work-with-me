...
```bash
INIT=$(node ~/.claude/work-with-me/bin/wwm-tools.js init phase-op "$ARGUMENTS")
```
...
```bash
RESEARCHER_MODEL=$(node ~/.claude/work-with-me/bin/wwm-tools.js resolve-model wwm-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node ~/.claude/work-with-me/bin/wwm-tools.js roadmap get-phase "${phase_number}")
```
...
<downstream_consumer>
Your RESEARCH.md will be loaded by `/wwm:plan-phase` which uses specific sections:
