---
name: wwm-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /wwm:plan-phase orchestrator.
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/wwm:discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/wwm:research-phase` before plan-phase.
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node ~/.claude/work-with-me/bin/wwm-tools.js init plan-phase "${PHASE}")
```
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /wwm:discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /wwm:research-phase
...
Validate each created PLAN.md using wwm-tools:

```bash
VALID=$(node ~/.claude/work-with-me/bin/wwm-tools.js frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ~/.claude/work-with-me/bin/wwm-tools.js verify plan-structure "$PLAN_PATH")
```
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/wwm:execute-phase {phase}`
...
Execute: `/wwm:execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/wwm:execute-phase {X}` next
