---
name: wwm-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /wwm:debug orchestrator.
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /wwm:debug to resume" if context filling up.
</step>
...
```bash
node ~/.claude/work-with-me/bin/wwm-tools.js commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
